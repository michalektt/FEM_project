...
public class vectorP extends Matrix {
    ...
    public vectorP(ReadTXT read, Jacobian jak, MatrixH_BC_2d hbc, MatrixC matrixC){
        ...
        createVecP(jak, hbc, matrixC);

    }

    private void createVecP(Jacobian jak, MatrixH_BC_2d hbc, MatrixC matrixC){
            ...
            for(int j=0; j<numberOfPoints; ++j){                        //która ściana
                for(int k=0; k<numberOfPoints; ++k) {                   //która funkcja kształtu
                    if(jak.getGrid().element[i].getIsOnEdge()[j]) {
                        if(j==0){       //góra
                            for(int m=0; m<2; ++m)
                                vectorP_PC[i][j][k] += (talfa * t * hbc.getN()[2][m][k]) * hbc.getDetJ()[i][1];
                        }
                        else if(j==1){  //dół
                            for(int m=0; m<2; ++m)
    ...
    /*    for(int i=0; i<numberOfElements; ++i){
            for(int j=0; j<numberOfPoints; ++j){                        //która ściana
                for(int k=0; k<numberOfPoints; ++k) {                   //która funkcja kształtu
                    vectorP_PC[i][j][k]+=(matrixC.getMatrixC()[i][j][k]/stepTime) * initialTemperature;
                }
            }
        }
*/
