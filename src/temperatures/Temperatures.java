...
public class Temperatures {
    ...
    //wyliczanie zmiany wektora P dla kolejnych iteracji
    private double[] countStepP(double inputStepTime, double[][] matrixC, double[] t0, double[] vectorP){

        double [] succeedingP = new double[this.vectorP.length];

        for(int i = 0; i< this.vectorP.length; i++) {
            for (int j = 0; j < this.vectorP.length; j++) {
                succeedingP[i] += (matrixC[i][j] / inputStepTime) * t0[j];
            }
    ...
    private double[] countEquation(double[][] finalH, double[] vecP){
        ...
        Matrix reverseMatrix = matrixH.inverse();
